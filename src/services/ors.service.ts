    ...
    return enqueue(
        async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `OpenRouteService error getting geocode. Status: ${response.status}`,
                    );
                }
                ...
                if (isGeocodingResponse(data)) {
                    ...
                    throw new Error(
                        "Received data does not match DirectionsResponse format",
                    );
                }
            } catch (error) {
    ...
    return enqueue(
        async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(
                        `OpenRouteService error getting route. Status: ${response.status}`,
                    );
                }
...
function isGeocodingResponse(
    data: GeocodingResponse,
): data is GeocodingResponse {
    return (
...
function isDirectionsResponse(
    data: DirectionsResponse,
): data is DirectionsResponse {
    return (
        data &&
